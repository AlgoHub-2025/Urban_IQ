"""
Waterway Type Classifier - Lahore
================================================
NOTE ON DATA: the `lat`/`lon` columns in waterways_lahore.csv are all 0 for
every row (a data export issue), so no real geospatial features exist.
The only informative signal is the waterway `name` itself (many are in
Urdu/Arabic script, some in English), so this model predicts the
the waterway (canal/stream/river) class purely from the name text
...
the best by weighted F1, refits on full data, and exports:
  ...
  2. predictions for every waterway in the dataset as JSON

Usage:
    python waterway_type_classifier.py --input waterways_lahore.csv \
        --output waterways_predictions.json
"""
...
def main():
    ...
    parser.add_argument("--input", default="waterways_lahore.csv")
    parser.add_argument("--output", default="waterways_predictions.json")
    parser.add_argument("--model-dir", default="models/waterways")
    parser.add_argument("--label-col", default="waterway")
    ...
    joblib.dump(final_pipe, f"{args.model_dir}/waterway_type_model.joblib")
        ...
        test_records.append(
            {
                "id": int(row["id"]),
                "name": row["name"],
                "actual_waterway": row[label_col],
                "predicted_waterway": test_preds[i],
    ...
    test_output = {
        "source": "waterway_type_classifier",
    ...
    # 2) Predictions for every waterway (sampled if huge; here we do the full set)
        ...
        all_records.append(
            {
                ...
                "name": row["name"],
                "actual_waterway": row[label_col],
                "predicted_waterway": full_preds[i],

"""
School/College/University Facility Classifier - Lahore
===============================================
Trains a classification model to predict facility `type` (school vs
college vs university) from its geographic location (lat/lon), using
engineered spatial features: distance from city center, quadrant, and
local point density.

Compares Logistic Regression, Random Forest, Gradient Boosting and
XGBoost with a stratified train/test split (classes: ~66% school /
19% college / 15% university), selects the best by F1-score, refits
on full data, and exports:
  1. test-set predictions (actual vs predicted) as JSON
  2. predictions for every facility in the dataset as JSON

Usage:
    python school_classifier.py --input schools_lahore.csv \
        --output schools_predictions.json
"""
...
def main():
    ...
    parser.add_argument("--input", default="schools_lahore.csv")
    parser.add_argument("--output", default="schools_predictions.json")
    parser.add_argument("--model-dir", default="models/schools")
    ...
    joblib.dump(final_model, f"{args.model_dir}/school_type_model.joblib")
